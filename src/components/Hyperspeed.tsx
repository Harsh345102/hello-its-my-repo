  ...
  useEffect(() => {
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          let laneIndex = Math.floor(Math.random() * options.lanesPerRoad);
          let carLane = laneIndex * laneWidth - options.roadWidth / 2 + laneWidth / 2;

          let carWidth = random(options.carWidthPercentage) * laneWidth;

          const carShiftX = random(options.carShiftX) * laneWidth;
          const offsetX = carLane + carShiftX;
      ...
      update(time) {
        this.mesh.material.uniforms.uTime.value = time * this.speed[1];
      }
    }
    ...
    class LightsSticks {
      ...
      init() {
        ...
        for (let i = 0; i < totalSticks; i++) {
          ...
          aOffset.push((options.roadWidth + options.islandWidth) / 2);
          aOffset.push(height / 2);
          aOffset.push(-i * stickoffset);
        ...
        const material = new THREE.ShaderMaterial({
          fragmentShader: stickFragment,
          vertexShader: stickVertex,
          uniforms: Object.assign(
            {
              uTravelLength: { value: options.length },
              uTime: { value: 0 }
    ...
    const carLightsVertex = `
      ...
      void main() {
        ...
        transformed.xy *= vec2(radius, myLength);
        transformed.z += myLength / 2.;
        transformed.xy *= -1.;
        float progress = mod(aOffset.z + uTime, uTravelLength) / uTravelLength;
        vec3 distortion = getDistortion(progress);
        transformed.x += uTime + aOffset.x + distortion.x;
        transformed.y += aOffset.y + distortion.y;
        transformed.z += -progress * uTravelLength + distortion.z;
    ...
    const stickVertex = `
      ...
      void main(){
        vec3 transformed = position.xyz;
        float width = aMetrics.r;
        float height = aMetrics.g;
        transformed.xy *= vec2(width, height);
        float progress = mod(aOffset.z + uTime, uTravelLength) / uTravelLength;
        vec3 distortion = getDistortion(progress);
        transformed.x += aOffset.x + distortion.x;
        transformed.y += aOffset.y + distortion.y;
        transformed.z += -progress * uTravelLength + distortion.z;
